...
import (
	...
	cache_package "github.com/max-fletcher/golang_web_server_boilerplate/internal/cache"
...
func NewRedisCache(client *redisClient.Client, isActive bool, defaultExp time.Duration) cache.Cache { // Return a struct that contains an instance of redis client
	return &redisCache{
		client:     client,
		isActive:   isActive,
...
func (cache *redisCache) Get(ctx context.Context, key string, dest any) error {
	...
	if err != nil {
		if errors.Is(err, redisClient.Nil) {
			return cache_package.ErrCacheMiss{ // if you don't do this, golang thinks of using "cache receiver"(struct whose method this is)
				Err: err,
			}
		}
...
func (cache *redisCache) DeleteByPrefix(ctx context.Context, prefix cache.CacheKeyValidPrefixes) error {
	if !cache.isActive {
		return nil
	}
